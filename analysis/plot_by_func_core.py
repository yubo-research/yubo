"""Internal helpers for :mod:`analysis.plot_by_func`."""
...
def load_traces_for_func(results_path, exp_dir, opt_names, func_name, num_dims):
    all_traces = []
...
def get_display_name(name, renames):
    return renames.get(name, name) if renames else name


def plot_func_subplot(ax, all_traces, opt_names, renames, func_name):
    if len(all_traces) == 0:
    ...
    for i_opt, opt_name in enumerate(sorted_opt_names):
        ax.errorbar(
            ...
            capsize=4,
            capthick=1,
            label=get_display_name(opt_name, renames),
            markersize=4,
            ecolor="black",
        )
    ...
    ax.set_xticks(x_pos)
    ax.set_xticklabels(
        [get_display_name(n, renames) for n in sorted_opt_names],
        rotation=45,
        ha="right",
        fontsize=8,
    )
...
def flatten_func_groups(func_groups):
    all_funcs = []
    group_labels = []
...
def grid_layout(num_funcs, max_cols=4, max_rows=8):
...
def hide_extra_axes(axs, num_funcs):
    if len(axs) > num_funcs:
        for ax in axs[num_funcs:]:
            ax.set_visible(False)


def safe_plot_func(ax, results_path, exp_dir, opt_names, func_name, num_dims, renames):
    try:
        all_traces = load_traces_for_func(results_path, exp_dir, opt_names, func_name, num_dims)
        plot_func_subplot(ax, all_traces, opt_names, renames, func_name)
...
def add_group_labels(fig, axs, group_labels, cols):
    current_group = None
    group_start_idx = 0
    for i_func in range(len(group_labels)):
        ...
        if current_group != group_name:
            if current_group is not None and group_start_idx < i_func:
                add_group_label(fig, axs, group_start_idx, i_func - 1, cols, current_group)
            current_group = group_name
            group_start_idx = i_func
    if current_group is not None and group_start_idx < len(group_labels):
        add_group_label(fig, axs, group_start_idx, len(group_labels) - 1, cols, current_group)


def add_group_label(fig, axs, start_idx, end_idx, cols, group_name):
...
def plot_by_func_grouped(
    ...
    for group_name, funcs in func_groups_map.items():
        if not funcs:
            continue
