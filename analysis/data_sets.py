...
def problems_in(exp_tag, num_arms, num_dim):
    return sorted(os.listdir(f"results/{exp_tag}/{num_arms}arm/{num_dim}d"))


def optimizers_in(exp_tag, problem, num_arms, num_dim):
    if problem == ".ipynb_checkpoints":
        pass
    else:
        return sorted(os.listdir(f"results/{exp_tag}/{num_arms}arm/{num_dim}d/{problem}/1iter"))


def all_in(exp_tag, num_arms, num_dim):
    optimizers = set()
    problems = problems_in(exp_tag, num_arms, num_dim)
    for problem in problems:
        for optimizer in optimizers_in(exp_tag, problem, num_arms, num_dim):
            optimizers.update(optimizers_in(exp_tag, problem, num_arms, num_dim))
...
def load_as_normalized_summaries(exp_tag, problem_names, optimizer_names, data_locator,num_arms, num_dim, i_only=None):
    normalized_summaries = {}
    count = {}
    for problem_name in problem_names:
        summaries = {}
        for optimizer_name in optimizer_names:
            fn = data_locator(exp_tag, problem_name, optimizer_name,num_arms, num_dim)
            try:
...
def agg_rank_summaries(exp_tag, problems, optimizers, data_locator,num_arms, num_dim, num_boot=100):
    summaries_by_opt = {}
    for problem in problems:
        summary = load_rank_summary(exp_tag, problem, optimizers, data_locator,num_arms, num_dim, num_boot)
...
def load_rank_summary(exp_tag, problem, optimizers, data_locator, num_arms, num_dim, num_boot=100):
    finals = []
    for optimizer in optimizers:
        # traces ~ num_replicates X num_rounds
        try:
            traces = load_traces(data_locator(exp_tag, problem, optimizer, num_arms, num_dim))
        except Exception as e:
