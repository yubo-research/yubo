...
def diversity_subsample(x, M, num_bins_per_dim=None, seed=None):
    ...
    for idx, cell_hash in enumerate(grid_hash):
        if cell_hash not in cell_to_points:
            cell_to_points[cell_hash] = []
        cell_to_points[cell_hash].append(idx)

    num_cells = len(cell_to_points)
    ...
    selected_indices = selected_indices[:M]
