...
def test_local_geometry_values_dataclass_semantics():
    first = LocalGeometryValues(
        delta_x=np.array([[1.0, -1.0]], dtype=float),
        y_neighbors=np.array([2.0], dtype=float),
        weights=np.array([1.0], dtype=float),
    )
    second = LocalGeometryValues(
        delta_x=np.array([[1.0, -1.0]], dtype=float),
        y_neighbors=np.array([2.0], dtype=float),
        weights=np.array([1.0], dtype=float),
    )
    np.testing.assert_allclose(first.delta_x, second.delta_x)
    np.testing.assert_allclose(first.y_neighbors, second.y_neighbors)
    np.testing.assert_allclose(first.weights, second.weights)
    assert "LocalGeometryValues" in repr(first)
