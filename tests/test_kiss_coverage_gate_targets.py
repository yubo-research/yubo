...
def test_kiss_cov_runner_main(monkeypatch, tmp_path):
    import common.config_toml as config_toml
    import rl.runner as runner
    import rl.runner_helpers as runner_helpers

    cfg_path = tmp_path / "rl.toml"
    cfg_path.write_text('[rl]\nalgo="dummy"\n[rl.dummy]\nseed=7\nexp_dir="tmp"\n')

    monkeypatch.setattr(runner_helpers, "split_config_and_args", lambda argv: (str(cfg_path), argv[1:]))
    monkeypatch.setattr(runner_helpers, "parse_runtime_args", lambda rest: SimpleNamespace(workers=1, workers_cli_set=False, cleaned=rest))
    monkeypatch.setattr(config_toml, "parse_set_args", lambda cleaned: {})
    monkeypatch.setattr(config_toml, "load_toml", lambda _path: {"rl": {"algo": "dummy", "dummy": {"seed": 7, "exp_dir": "tmp"}}})
    monkeypatch.setattr(config_toml, "apply_overrides", lambda cfg, overrides: None)
