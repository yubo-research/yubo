...
def _stagger_sampler_and_dim(*, seed, num_samples):
    torch.manual_seed(seed)
    model, X_max = gp_parabola()
    X_max = torch.atleast_2d(X_max)
    num_dim = X_max.shape[-1]
    pss = StaggerThompsonSampler(model, X_max, num_samples=num_samples)
    return pss, num_dim


def test_stagger_thompson_sampler():
    pss, num_dim = _stagger_sampler_and_dim(seed=17, num_samples=16)
    for _ in range(3):
...
def test_stagger_thompson_sampler_ts_chain():
    pss, num_dim = _stagger_sampler_and_dim(seed=42, num_samples=8)
