...
from rl.core.torchrl_runtime import (
    TorchRLRuntime,
...
def test_select_device_auto_prefers_mps_when_cuda_unavailable(monkeypatch):
    monkeypatch.setattr("rl.core.runtime.torch.cuda.is_available", lambda: False)
    monkeypatch.setattr("rl.core.runtime.mps_is_available", lambda: True)
    assert select_device("auto").type == "mps"


def test_select_device_cpu_fallback(monkeypatch):
    monkeypatch.setattr("rl.core.runtime.torch.cuda.is_available", lambda: False)
    monkeypatch.setattr("rl.core.runtime.mps_is_available", lambda: False)
