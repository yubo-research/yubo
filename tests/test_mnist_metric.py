...
def test_mnist_metric_returns_negative_ce_and_tensor(tmp_path):
    ...
    metric = MNISTMetric(data_root=str(tmp_path), batch_size=16, seed=0, train=True, num_workers=0)
...
def test_mnist_metric_device_migration(tmp_path):
    ...
    metric = MNISTMetric(data_root=str(tmp_path), batch_size=8, seed=1, train=True, num_workers=0)
