...
def test_kiss_cov_direct_eval_config_and_uhd_setup(monkeypatch):
    ...
    got = build_eval_env_conf(
        cfg,
        obs_mode="vector",
        is_atari_env_tag_fn=lambda tag: False,
        resolve_gym_env_name_fn=lambda tag: ("CartPole-v1", {}),
    )
    ...
    monkeypatch.setattr(
        "ops.uhd_setup_monolith_support._load_build_problem",
        lambda: lambda *args, **kwargs: fake_problem_simple,
    )
    called = {"simple": 0, "bszo": 0}
    monkeypatch.setattr(
        "ops.uhd_setup._run_simple_torch",
    ...
    monkeypatch.setattr(
        "ops.uhd_setup_monolith_support._load_build_problem",
        lambda: lambda *args, **kwargs: fake_problem_bszo,
    )
    monkeypatch.setattr(
        "ops.uhd_setup_monolith_support._make_torch_env",
        lambda problem, **kwargs: SimpleNamespace(torch_env=lambda: SimpleNamespace(module=torch.nn.Linear(2, 1))),
    )
    monkeypatch.setattr("ops.uhd_setup_monolith_support._get_device", lambda: torch.device("cpu"))
    monkeypatch.setattr(
        "ops.uhd_setup_monolith_bszo._run_bszo_iterations",
        lambda *args, **kwargs: called.__setitem__("bszo", called["bszo"] + 1),
    )
    monkeypatch.setattr(
        "optimizer.uhd_bszo.UHDBSZO",
...
def test_kiss_cov_direct_fit_main_episode_rollout_backbone_and_turbo(monkeypatch):
    ...
    called = {"fit": 0}
    monkeypatch.setattr(
        "ops.fit_mnist.fit_mnist",
        lambda **kwargs: called.__setitem__("fit", called["fit"] + 1),
    )
    fit_main(1, 8, 1e-3, 1)
    assert called["fit"] == 1
