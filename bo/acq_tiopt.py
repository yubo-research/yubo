...
class AcqTIOpt(MCAcquisitionFunction):
    def __init__(
        ...
        **kwargs
        ...
        self.X_samples = torch.cat((self.X_samples, self._noisy_maxes(num_X_samples)), axis=0)
    ...
    def _integrated_variance(self, num_var_samples):
        X_0 = self.model.train_inputs[0]
        num_dim = X_0.shape[-1]
        X = torch.rand(size=(num_var_samples, num_dim))
    ...
    def _calc_weights(self, X_samples, num_ts_samples, b_joint_sampling):
        ...
        if False:
            # v = max(1e-6, min(1.0, self._integrated_variance(num_var_samples)))
            # p_best = p_best ** (1.0 / v)
            # p_best = p_best / p_best.sum()
            pass
