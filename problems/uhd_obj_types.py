...
class UHDVectorObjective(Protocol):
    ...
    def make_policy(self, x: Any): ...

    def evaluate(self, x: Any, *, seed: int) -> tuple[float, float]: ...

    def evaluate_many(self, x_batch: Any, *, seed: int) -> tuple[np.ndarray, np.ndarray]: ...

    def configure_embedding(self, num_probes: int) -> None: ...

    def embed_many(self, x_batch: Any) -> np.ndarray: ...
