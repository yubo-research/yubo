...
# 0  xi ∈ [-5.12, 5.12], for all i = 1, …, d. {d dimensions}
class Sphere:
...
# 1 xi ∈ [-32.768, 32.768] ackley result [0,25] {d dimensions}
class Ackley:
    def __init__(self, seed, num_dim):
        rng = np.random.default_rng(seed)
        self._x_0 = rng.uniform(size=(num_dim,))
    ...
    def __call__(self, x):
        x = x * 30 - self._x_0
        result = -20.0 * np.exp(-0.2 * np.sqrt(0.5 * np.sum(x**2))) - np.exp(0.5 * np.sum(np.cos(2 * np.pi * x))) + np.e + 20
        return -result


# 2  xi ∈ [-4.5, 4.5], for all i = 1, 2. Beale result [0, 4.5*10^5] {2 dimensions}
class Beale:
    def __init__(self, seed, num_dim):
        rng = np.random.default_rng(seed)
        self._x_0 = rng.uniform(size=(num_dim,))
...
# 3 x1 ∈ [-5, 10], x2 ∈ [0, 15]. result [0,350] Branin {2 dimensions}
class Branin:
    def __init__(self, seed, num_dim):
        rng = np.random.default_rng(seed)
        self._x_0 = rng.uniform(size=(num_dim,))
...
# 4 x1 ∈ [-15, -5], x2 ∈ [-3, 3]. result [0,250] Bukin {2 dimensions}
class Bukin:
    def __init__(self, seed, num_dim):
        rng = np.random.default_rng(seed)
        self._x_0 = rng.uniform(size=(num_dim,))
...
# 5 xi ∈ [-10, 10], for all i = 1, 2. result[-2.5,-0.5] CrossInTray {2 dimensions}
class CrossInTray:
    def __init__(self, seed, num_dim):
        rng = np.random.default_rng(seed)
        self._x_0 = rng.uniform(size=(num_dim,))
...
# 6  xi ∈ [-5.12, 5.12], for all i = 1, 2. result[-1,0] DropWave {2 dimensions}
class DropWave:
    def __init__(self, seed, num_dim):
        rng = np.random.default_rng(seed)
        self._x_0 = rng.uniform(size=(num_dim,))
...
# 7 xi ∈ [-10, 10] result [0,9*10^4] DixonPrice {d dimensions}
class DixonPrice:
    def __init__(self, seed, num_dim):
        rng = np.random.default_rng(seed)
        self._x_0 = rng.uniform(size=(num_dim,))
...
# 8 EggHolder xi ∈ [-512, 512], for all i = 1, 2.result [-1000,1500] {2 dimensions}
class EggHolder:
    def __init__(self, seed, num_dim):
        rng = np.random.default_rng(seed)
        self._x_0 = rng.uniform(size=(num_dim,))
...
# 9 Griewank xi ∈ [-600, 600] , for all i = 1, …, d. result [0,200] {d dimensions}
class Griewank:
    def __init__(self, seed, num_dim):
        rng = np.random.default_rng(seed)
        self._x_0 = rng.uniform(size=(num_dim,))
...
# 10 GrLee12 one-dimention x ∈ [0.5, 2.5]. result [-1,6] {1 dimension}
class GrLee12:
    def __init__(self, seed, num_dim):
        rng = np.random.default_rng(seed)
        self._x_0 = rng.uniform(size=(num_dim,))
...
class Hartmann:
    def __init__(self, seed, num_dim):
        ...
        if num_dim == 4 or num_dim == 6:
            self.A = np.array((10, 3, 17, 3.5, 1.7, 8, 0.05, 10, 17, 0.1, 8, 14, 3, 3.5, 1.7, 10, 17, 8, 17, 8, 0.05, 10, 0.1, 14)).reshape(4, 6)
...
# 12 HolderTable xi ∈ [-10, 10], for all i = 1, 2. result [-20,0] {2 dimensions}
class HolderTable:
    def __init__(self, seed, num_dim):
        rng = np.random.default_rng(seed)
        self._x_0 = rng.uniform(size=(num_dim,))
...
# 13 Levy  xi ∈ [-10, 10], for all i = 1, …, d. result [0,100] {d dimensions}
class Levy:
    def __init__(self, seed, num_dim):
        rng = np.random.default_rng(seed)
        self._x_0 = rng.uniform(size=(num_dim,))
...
# 14 Michalewicz xi ∈ [0, π], for all i = 1, …, d. result[-1.8,0]  {d dimensions}
class Michalewicz:
    def __init__(self, seed, num_dim):
        rng = np.random.default_rng(seed)
        self._x_0 = rng.uniform(size=(num_dim,))
...
# 15 Powell xi ∈ [-4, 5], for all i = 1, …, d. {d dimensions}
class Powell:
    def __init__(self, seed, num_dim):
        rng = np.random.default_rng(seed)
        self._x_0 = rng.uniform(size=(num_dim,))
...
# 16 Rastrigin  xi ∈ [-5.12, 5.12], for all i = 1, …, d result[0,90] {d dimensions}
class Rastrigin:
    def __init__(self, seed, num_dim):
        rng = np.random.default_rng(seed)
        self._x_0 = rng.uniform(size=(num_dim,))
    ...
    def __call__(self, x):
        ...
        d = len(x)
        result = 10 * d + np.sum(x**2 - 10 * np.cos(np.pi * 2 * x))
        return -result


# 17 Rosenbrock  xi ∈ [-2.048, 2.048], for all i = 1, …, d. result [0,18*10^4] {d dimensions}, d>=2
class Rosenbrock:
    def __init__(self, seed, num_dim):
        rng = np.random.default_rng(seed)
        self._x_0 = rng.uniform(size=(num_dim,))
...
# 18 Shubert xi ∈ [-5.12, 5.12], for all i = 1, 2. result[-200,300] {2 dimensions}
class Shubert:
    def __init__(self, seed, num_dim):
        rng = np.random.default_rng(seed)
        self._x_0 = rng.uniform(size=(num_dim,))
...
# 19 Shekel  xi ∈ [0, 10], for all i = 1, 2, 3, 4. {4 dimensions}
class Shekel:
    def __init__(self, seed, num_dim):
        rng = np.random.default_rng(seed)
        self._x_0 = rng.uniform(size=(num_dim,))
...
# 20 SixHumpCamel x1 ∈ [-3, 3], x2 ∈ [-2, 2]. result [-50,200] {2 dimensions}
class SixHumpCamel:
    def __init__(self, seed, num_dim):
        rng = np.random.default_rng(seed)
        self._x_0 = rng.uniform(size=(num_dim,))
    ...
    def __call__(self, x):
        ...
        x0 = x[0] * 3
        x1 = x[1] * 2
        result = (4 - 2.1 * x0**2 + x0**4 / 3) * x0**2 + x0 * x1 + (4 * x1**2 - 4) * x1**2
        return -result - 50


# 21 StybTang  xi ∈ [-5, 5], for all i = 1, …, d. result [-100,250] {d dimensions}
class StybTang:
    def __init__(self, seed, num_dim):
        rng = np.random.default_rng(seed)
        self._x_0 = rng.uniform(size=(num_dim,))
...
# 22 ThreeHumpCamel xi ∈ [-5, 5], for all i = 1, 2. result[0,10][0,1000] {2 dimensions}
