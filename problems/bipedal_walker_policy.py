...
class BipedalWalkerPolicy:
    ...
    def _compute_control(self, s, hip_targ, knee_targ):
        ...
        joint_params = [(hip_targ[0], 4, 5, hip_todo, 0), (hip_targ[1], 9, 10, hip_todo, 1),
                        (knee_targ[0], 6, 7, knee_todo, 0), (knee_targ[1], 11, 12, knee_todo, 1)]
    ...
    def __call__(self, state):
        ...
        a = np.clip(self._act_scale * np.array([hip_todo[0], knee_todo[0], hip_todo[1], knee_todo[1]], dtype=np.float64), -1.0, 1.0)
        self._t_in_state += 1
        return a
