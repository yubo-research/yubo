...
def iterate_nanoegg(designer, state, _data, num_arms: int, *, telemetry=None) -> IterateResult:
    _ = _data
    population = _check_population(designer, int(num_arms))
    result = _collect_population(designer, state, population)
    prop_dt = _apply_update(designer, state, result.directions, result.raw_scores, population)
    datum, policy_eval_dt = _evaluate_updated_policy(designer, state, population)
...
def _collect_population(designer, state, population: int) -> _PopulationResult:
    t_eval = time.time()
    directions: list[np.ndarray] = []
    raw_scores: list[float] = []
    ...
    _log(
        designer,
        f"iter={state.epoch} evaluating population={population} dim={state.x.size} batch_size={designer._nanoegg_batch_size}",
    )
    ...
    for pair_idx in range(population // 2):
        ...
        for sign in (1.0, -1.0):
            pending_x.append(state.x + sign * designer._current_sigma() * direction)
...
def _apply_update(
    ...
    updates, state.opt_state = designer._nanoegg_tx.update(grad_j, state.opt_state, x_j)
    updated_x = designer._nanoegg_apply_updates(x_j, updates)
    state.x = np.asarray(designer._objective.jax.block_until_ready(updated_x), dtype=np.float64)
    return time.time() - t_prop


def _evaluate_updated_policy(designer, state, population: int):
    t_eval = time.time()
    eval_mean, eval_se = designer._objective.evaluate(state.x, seed=(state.epoch + 1) * 1_000_003)
    policy = designer._policy.with_snapshot(designer._objective.make_policy(state.x))
    datum = Datum(
        designer,
        policy,
        None,
...
def update_best_and_telemetry(designer, state, datum: Datum, prop_dt: float, telemetry) -> None:
    if state.best_datum is None or datum.trajectory.get_decision_rreturn() > state.best_datum.trajectory.get_decision_rreturn():
        state.best_datum = datum
...
def _sample_direction(designer, state, pair_idx: int) -> np.ndarray:
    direction = designer._objective.sample_eggroll_noiser_noise(
        state.x,
        seed=direction_seed(designer, state, pair_idx),
        noiser_name="eggroll",
        rank=designer._rank,
...
def _flush_pending(
    ...
    *,
    ...
    mus, _ses = designer._objective.evaluate_many(batch, seed=(state.epoch * population) + pending_start)
    raw_scores.extend(float(v) for v in np.asarray(mus, dtype=np.float64).reshape(-1))
    done = len(raw_scores)
    pending_x.clear()
    return done, _maybe_log_progress(designer, state, done, next_log_at, population, t_eval)


def _maybe_log_progress(designer, state, done: int, next_log_at: int, population: int, t_eval: float) -> int:
    if done < next_log_at and done != population:
        return next_log_at
    _log(
        designer,
        f"iter={state.epoch} evaluated={done}/{population} elapsed={time.time() - t_eval:.1f}s",
    )
