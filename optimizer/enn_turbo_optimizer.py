...
class TurboOptimizer:
    ...
    def _ask_normal(self, num_arms: int, *, is_fallback: bool = False) -> np.ndarray:
        ...
        x_low = self._bounds[:, 0]
        x_high = self._bounds[:, 1]
        return np.clip(x_new, x_low, x_high)
    ...
    def _trim_trailing_obs(self) -> None:
        ...
        # Align TR bookkeeping with the retained observation window. Otherwise
        # prev_num_obs can stay larger than the trimmed buffer and suppress
        # future length updates when len(y_obs) appears unchanged.
