...
def _create_optimizer_auto(bounds, config, rng):
    ...
    try:
        enn = _im("enn")
        return enn.create_optimizer(bounds=bounds, config=config, rng=rng)
    except ImportError as exc:
        # Modal/remote images may not have enn_rust; fall back to Python optimizer.
        _logger.warning("Rust backend unavailable (%s); falling back to Python backend", exc)
...
class TurboENNDesigner:
    ...
    def _parse_candidate_rv(self):
        CandidateRV = _im("enn.turbo.config.candidate_rv").CandidateRV
    ...
    def _parse_acq_type(self):
        AcqType = _im("enn.turbo.config.acq_type").AcqType
        try:
    ...
    def _make_trust_region(self, num_metrics: int | None):
        ...
        TurboTRConfig = tr.TurboTRConfig
        NoTRConfig = tr.NoTRConfig
        MorboTRConfig = tr.MorboTRConfig
        ...
        if self._tr_type == "morbo":
            ...
            MultiObjectiveConfig = tr.MultiObjectiveConfig
    ...
    def _make_config(self, num_init: int, num_metrics: int | None):
        enn_surrogate = _im("enn.turbo.config.enn_surrogate_config")
        ENNFitConfig = enn_surrogate.ENNFitConfig
        ENNSurrogateConfig = enn_surrogate.ENNSurrogateConfig
        cgen = _im("enn.turbo.config.candidate_gen_config")
        CandidateGenConfig = cgen.CandidateGenConfig
        raasp = _im("enn.turbo.config.raasp_driver")
        ...
        if self._turbo_mode == "turbo-enn":
            ...
            return factory.turbo_enn_config(
                enn=enn,
    ...
    def _init_optimizer(self, data, num_arms):
        ...
        np = _im("numpy")
        ab = _im("common.all_bounds")
        bounds = np.array([[ab.x_low, ab.x_high]] * num_dim)
        ...
        _logger.debug("Optimizer type: %s (%s backend)", opt_type, backend)
    ...
    def _tell_new_data(self, new_data):
        if self._tr_type != "morbo":
            da = _im("optimizer.designer_asserts")
