        ...
        def posterior(self, X: torch.Tensor):
            ...
            class _P:
                ...
                def sample(self, sample_shape: torch.Size):
                    # ENNNormal.sample returns numpy array shaped (N, 1, S)
                    samples_np = self._mvn.sample(sample_shape)
                    samples_t = _to_torch(samples_np, like=self._X_like)  # (N, 1, S)
                    return samples_t.permute(2, 0, 1).contiguous()  # (S, N, 1)
