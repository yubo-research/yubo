...
class AcqVHD:
    ...
    def _thompson_sample(self, x, num_arms, num_candidates_per_arm):
        num_dim = x.shape[-1]
    ...
    def _find_isolated(self, x_a, num_samples):
        ...
        for _ in range(num_samples):
            xs = np.random.uniform(size=(100 * num_samples, self._num_dim))
            x = most_isolated(enn, xs)
            x_isolated.append(x)
            enn.add(x, 0)

        return np.vstack(x_isolated)

    def _num_explore(self, num_arms):
        p_explore = 0.25  # min(1.0, 1 / (1 + len(self._X_train)))
        return binom(num_arms, p_explore).rvs()
