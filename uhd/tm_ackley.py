...
class TMAckley(nn.Module):
    ...
    def __init__(self, num_dim: int, num_active: int, seed: int) -> None:
        ...
        self.parameters_ = nn.Parameter(self.lb + (self.ub - self.lb) * torch.rand(num_dim, generator=g))
        self.register_buffer("active_idx", idx)
        self.register_buffer("x_0_active", self.lb + (self.ub - self.lb) * torch.rand(num_active, generator=g))
