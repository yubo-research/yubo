...
from rl.offpolicy import engine_utils as offpolicy_engine_utils
from rl.precision import PrecisionController
...
_SAC_COMPILE_SKIP = frozenset({"0", "false", "no", "skip", "off"})


# SAC_COMPILE: 0/false/skip = no compile; 1/default = default; reduce-overhead, max-autotune, aot_eager = pass-through.
def _sac_compile_mode() -> str | None:
    """Parse SAC_COMPILE env var. None = skip, else mode/backend string."""
    raw = os.environ.get("SAC_COMPILE", "").strip().lower()
    if not raw or raw in _SAC_COMPILE_SKIP:
        return None
...
def _maybe_compile(module: torch.nn.Module, name: str, *, device: torch.device) -> torch.nn.Module:
    """Compile module if SAC_COMPILE allows; fallback to eager on failure."""
    if device.type == "mps":
        return module
    mode = _sac_compile_mode()
    if mode is None:
        return module
    # Mac CPU: inductor fails; reduce-overhead avoids it and is ~3% faster than eager (S6).
    if mode == "default" and device.type == "cpu":
    ...
    try:
        ...
        _log.warning("SAC compile failed for %s (%s), using eager: %s", name, mode, e)
        return module
...
def _build_training_components(
    ...
    *,
    ...
    modules.actor = _maybe_compile(modules.actor, "actor", device=device)
    modules.q1 = _maybe_compile(modules.q1, "q1", device=device)
    modules.q2 = _maybe_compile(modules.q2, "q2", device=device)
    ...
    replay = make_replay_buffer(
        ...
        device=device if replay_backend == "cuda" else None,
    )
...
def _state_payload(modules, optimizers, replay, state: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "actor_state": actor_snapshot,
        "actor_backbone": actor_snapshot["backbone"],
        "actor_head": actor_snapshot.get("head"),
...
def _restore_if_requested(config: SACConfig, modules, optimizers, replay, state: Any, *, device: torch.device) -> None:
    ...
    actor_snap = loaded.get("actor_state") or {
        "backbone": loaded["actor_backbone"],
        "head": loaded["actor_head"],
        "obs_scaler": loaded.get("obs_scaler"),
    }
    restore_actor_state(modules, actor_snap)
...
def _train_loop(
    ...
    *,
    ...
    while state.global_step < total_steps:
        ...
        nxt_obs_np, reward_np, terminated_np, truncated_np, infos = envs.step(action_env)
        # Use terminated only for Bellman target: truncated episodes should bootstrap from
        # next state (final_observation); terminated episodes should not.
        done_for_target = np.asarray(terminated_np, dtype=np.float32)
        # Use final_observation for truncated episodes (Gymnasium convention; CleanRL alignment).
        # SAC_USE_FINAL_OBSERVATION=0 disables for A/B testing.
        use_final_obs = os.environ.get("SAC_USE_FINAL_OBSERVATION", "1") in ("1", "true", "yes")
        real_next_obs = np.asarray(nxt_obs_np, dtype=np.float32).copy()
        if use_final_obs and "final_observation" in infos:
            for idx in range(num_envs):
                if truncated_np[idx]:
                    real_next_obs[idx] = infos["final_observation"][idx]
        nxt_obs_for_replay = prepare_obs_np(real_next_obs, obs_spec=obs_spec)
        replay.add_batch(obs_batch, action_norm, reward_np, nxt_obs_for_replay, done_for_target)
        obs_batch = prepare_obs_np(nxt_obs_np, obs_spec=obs_spec)
        ...
        if should_update and replay.size >= int(config.batch_size):
            ...
            def _run_one_update() -> None:
                try:
                    with precision_controller.autocast():
                        actor_loss, critic_loss, alpha_loss = sac_update(config, modules, optimizers, replay, device=device)
                except Exception as exc:
                    if precision_controller.maybe_demote_on_runtime_error(exc, component="SAC"):
                        actor_loss, critic_loss, alpha_loss = sac_update(config, modules, optimizers, replay, device=device)
                    else:
                        raise
                state.total_updates += 1
                state.last_loss_actor = actor_loss
                state.last_loss_critic = critic_loss
                state.last_loss_alpha = alpha_loss
...
def train_sac_puffer_impl(config: SACConfig) -> TrainResult:
    ...
    precision_controller = PrecisionController.from_config(getattr(config, "precision", None), device=device)
...
def register() -> None:
    ...
    register_algo("sac", SACConfig, train_sac_puffer)
