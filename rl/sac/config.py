...
class SACConfig:
    ...
    gac_action_radius: float = 2.5
    gac_adaptive_scale: bool = False
    ...
    def to_header_dict(self) -> dict:
        """Config dict for run header display. Omits GAC-only fields when actor_type != gac."""
        out = dataclasses.asdict(self)
        if str(self.actor_type).strip().lower() != "gac":
            out.pop("gac_action_radius", None)
            out.pop("gac_adaptive_scale", None)
        return out
