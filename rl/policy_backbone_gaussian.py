...
from rl.policy_backbone_utils import ensure_env_spaces, obs_space_from_env_conf
...
class GaussianActorBackbonePolicy(PolicyParamsMixin, nn.Module):
    def __init__(
        ...
        *,
        ...
        num_state = int(obs_space_from_env_conf(env_conf).shape[0])
    ...
    def get_params(self):
        with torch.inference_mode():
            flat = np.concatenate([p.data.detach().cpu().numpy().reshape(-1) for p in self.parameters()])
    ...
    def set_params(self, flat_params):
        ...
        with torch.inference_mode():
            for p in self.parameters():
                n = p.numel()
                arr = flat[offset : offset + n].reshape(p.shape)
                p.copy_(torch.from_numpy(arr).to(dtype=p.dtype))
                offset += n
