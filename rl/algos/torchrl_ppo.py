...
from rl.algos.ppo_checkpointing import (
    _append_jsonl,
    _checkpoint_paths,
    _load_checkpoint,
    _save_checkpoint,
...
class _CriticNet(nn.Module):
    def __init__(self, backbone: nn.Module, head: nn.Module, obs_scaler: _ObsScaler):
        super().__init__()
        self.backbone = backbone
        self.head = head
        self.obs_scaler = obs_scaler
...
class _EvalPolicy:
    ...
    def __call__(self, state):
        ...
        with torch.no_grad():
            feats = self._backbone(self._obs_scaler(state_t))
            loc = self._head(feats)
            action = torch.tanh(loc)
        return action.squeeze(0).detach().cpu().numpy()


def _select_device(config: PPOConfig) -> torch.device:
...
def _obs_scale_from_env(env_conf):
...
def _build_env_setup(config: PPOConfig) -> _EnvSetup:
    ...
    lb, width = _obs_scale_from_env(env_conf)
...
def _build_modules(config: PPOConfig, env: _EnvSetup, *, device: torch.device) -> _Modules:
    ...
    obs_scaler = _ObsScaler(env.obs_lb, env.obs_width)
...
def _resume_if_requested(
    ...
    *,
    ...
    loaded = _load_checkpoint(resume_path, device=device)
...
def _capture_actor_state(modules: _Modules) -> dict:
    return {
        "backbone": modules.actor_backbone.state_dict(),
        "head": modules.actor_head.state_dict(),
        "log_std": modules.log_std.detach().cpu().numpy(),
    }


def _restore_actor_state(modules: _Modules, state: dict, *, device: torch.device) -> None:
    modules.actor_backbone.load_state_dict(state["backbone"])
    modules.actor_head.load_state_dict(state["head"])
    modules.log_std.data.copy_(torch.as_tensor(state["log_std"], device=device))
    ...
    *,
    ...
    if not do_eval:
        if do_log:
            ...
            print(
                f"[rl/ppo/torchrl] iter={iteration:04d}/{training.num_iterations} step={iteration * training.frames_per_batch} time={elapsed:.1f}s",
                flush=True,
            )
        return
    ...
    if state.last_eval_return > state.best_return:
        ...
        state.best_actor_state = _capture_actor_state(modules)

    state.last_heldout_return = None
    if config.num_denoise_passive_eval is not None and state.best_actor_state is not None:
        current = _capture_actor_state(modules)
        _restore_actor_state(modules, state.best_actor_state, device=device)
        best_eval_policy = _EvalPolicy(modules.actor_backbone, modules.actor_head, modules.obs_scaler, device=device)
        ...
        _restore_actor_state(modules, current, device=device)
    ...
    _append_jsonl(training.metrics_path, record)
    if do_log:
        ...
        print(
            ...
            f" step={iteration * training.frames_per_batch}"
...
def _checkpoint_payload(
    ...
    *,
    ...
    return {
        ...
        "global_step": int(iteration * training.frames_per_batch),
...
def _maybe_checkpoint(
    ...
    *,
    ...
    if not config.checkpoint_interval or iteration % int(config.checkpoint_interval) != 0:
        return
    ckpt_path, latest_path = _checkpoint_paths(training.exp_dir, iteration)
    payload = _checkpoint_payload(training, modules, state, iteration=iteration)
    _save_checkpoint(ckpt_path, payload)
    _save_checkpoint(latest_path, payload)


def _final_checkpoint(config: PPOConfig, training: _TrainingSetup, modules: _Modules, state: _TrainState) -> None:
    if not config.checkpoint_interval:
        return
    ckpt_path, latest_path = _checkpoint_paths(training.exp_dir, training.num_iterations)
    payload = _checkpoint_payload(training, modules, state, iteration=training.num_iterations)
    _save_checkpoint(ckpt_path, payload)
    _save_checkpoint(latest_path, payload)
...
def train_ppo(config: PPOConfig) -> TrainResult:
    ...
    for iteration, batch in enumerate(collector, start=state.start_iteration + 1):
        ...
        _maybe_checkpoint(config, training, modules, state, iteration=iteration)

    collector.shutdown()
    _final_checkpoint(config, training, modules, state)
    return TrainResult(
        best_return=float(state.best_return),
        last_eval_return=float(state.last_eval_return),
        last_heldout_return=state.last_heldout_return,
