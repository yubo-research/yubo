...
def maybe_eval_and_update_state(
    ...
    *,
    ...
    eval_noise = __import__("rl.algos.eval_noise", fromlist=["build_eval_plan"])
...
def maybe_render_videos(
    ...
    *,
    ...
    if getattr(eval_env_conf, "gym_conf", None) is None:
        print(
            f"[rl/ppo/puffer] video disabled for non-gym env: {config.env_tag}",
            flush=True,
        )
        return
...
def validate_eval_config(config) -> None:
    ...
    if config.eval_noise_mode is not None:
        eval_noise = __import__("rl.algos.eval_noise", fromlist=["normalize_eval_noise_mode"])
