...
from rl.pufferlib.offpolicy.eval_utils import TrainState, append_eval_metric, due_mark, log_if_due, maybe_eval, render_videos_if_enabled
from rl.pufferlib.offpolicy.replay import make_replay_buffer, resolve_replay_backend

from .config import TrainResult, WPOConfig
from .model_utils import build_modules, build_optimizers, use_actor_state, wpo_update

__all__ = ["WPOConfig", "TrainResult", "register", "train_wpo_puffer", "train_wpo_puffer_impl"]


def _log_header(config: WPOConfig, obs_batch: np.ndarray, env_setup, obs_spec, *, device: torch.device) -> None:
...
def _init_run_artifacts(config: WPOConfig) -> tuple[Path, Path, Any]:
    return init_run_artifacts(exp_dir=str(config.exp_dir), config_dict=config.to_dict())


def _init_runtime(config: WPOConfig):
    return init_runtime(config, build_env_setup_fn=build_env_setup, seed_everything_fn=seed_everything, resolve_device_fn=resolve_device)


def _build_training_components(config: WPOConfig, env_setup, obs_spec, obs_batch: np.ndarray, *, device: torch.device):
...
def _state_payload(modules, optimizers, replay, state: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "dual_optimizer": optimizers.dual_optimizer.state_dict(),
...
def _restore_if_requested(config: WPOConfig, modules, optimizers, replay, state: Any, *, device: torch.device) -> None:
    ...
    modules.actor_target.load_state_dict(loaded["actor_target"])
    ...
    if loaded.get("log_alpha_mean") is not None:
        modules.log_alpha_mean.data.copy_(loaded["log_alpha_mean"].to(device=device, dtype=modules.log_alpha_mean.dtype))
    if loaded.get("log_alpha_stddev") is not None:
        modules.log_alpha_stddev.data.copy_(loaded["log_alpha_stddev"].to(device=device, dtype=modules.log_alpha_stddev.dtype))
    ...
    optimizers.dual_optimizer.load_state_dict(loaded["dual_optimizer"])
    ...
    while state.global_step < total_steps:
        ...
        if should_update and replay.size >= int(config.batch_size):
            for _ in range(int(max(1, config.updates_per_step))):
                actor_loss, critic_loss, dual_loss, _alpha_mean, _alpha_stddev = wpo_update(config, modules, optimizers, replay, device=device)
                state.total_updates += 1
                state.last_loss_actor = float(actor_loss)
                state.last_loss_critic = float(critic_loss)
                state.last_loss_alpha = float(dual_loss)
        ...
        state.ckpt_mark = checkpoint_mark_if_due(
            ...
            save_fn=lambda: checkpoint_manager.save_both(_state_payload(modules, optimizers, replay, state), iteration=int(state.global_step)),
        )


def train_wpo_puffer_impl(config: WPOConfig) -> TrainResult:
...
def train_wpo_puffer(config: WPOConfig) -> TrainResult:
    return train_wpo_puffer_impl(config)


def register() -> None:
    from rl.registry import register_algo

    register_algo("wpo", WPOConfig, train_wpo_puffer, backend="pufferlib")
