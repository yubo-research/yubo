...
def normalize_action_bounds(low: np.ndarray, high: np.ndarray, dim: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    high_v = np.maximum(high_v, low_v + 1e-06)
    return (low_v, high_v)
...
def unscale_action_from_env(action: np.ndarray, low: np.ndarray, high: np.ndarray, *, clip: bool = True) -> np.ndarray:
    ...
    width = np.maximum(high_v - low_v, 1e-08)
...
def unscale_action_tensor_from_env(action: torch.Tensor, low: torch.Tensor, high: torch.Tensor, *, clip: bool = True) -> torch.Tensor:
    width = (high - low).clamp(min=1e-08)
...
def scale_action_tensor_to_env(action: torch.Tensor, low: torch.Tensor, high: torch.Tensor, *, clip: bool = True) -> torch.Tensor:
