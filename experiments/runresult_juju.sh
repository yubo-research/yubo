...
for ITER in 50 100 500
    do
    for Q_NUM in 1 2 5 10 20 50 100
        do
        for TAG in f:ackley-10d
        ...
        do 
            ...
            do
                echo "Start" $TAG $TTYPE $Q_NUM "arms" $ITER "Iters"
                # mkdir -p results/${DDIR}/${TAG}
                python experiments/run_result1.py ${TAG} ${TTYPE} ${Q_NUM} ${ITER} >/dev/null 2>&1
                echo "Finish"
            done
        done
    done
