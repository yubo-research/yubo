"""Per-checkpoint fit-ind JSON completeness for ENN Modal batches."""
...
_FIT_IND_META_REQUIRED: tuple[str, ...] = (
    "D",
    "function_name",
    "problem_seed",
    "data_seed",
...
def fit_ind_result_json_complete(
    dest: str | Path,
    ...
    *,
    ...
    try:
        ...
        fv, lv = payload["fit_seconds"], payload["log_likelihood"]
    except (OSError, KeyError, TypeError, ValueError, json.JSONDecodeError):
        return False
    tn = tuple(int(n) for n in expected_n)
    if n_vals != tn or len(fv) != len(tn) or len(lv) != len(tn):
        return False
    if not isinstance(meta, dict):
        return False
    if any(key not in meta for key in _FIT_IND_META_REQUIRED):
        return False
